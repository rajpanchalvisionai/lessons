...
#     results = db.similarity_search_with_relevance_scores(state['question'], k=10)
#     print(f"Initial retrieval results: {len(results)}")
#     for i, (doc, score) in enumerate(results):
#         print(f"Chunk {i+1}: Score={score:.2f}, Content={doc.page_content[:100]}")
#     file_path = "/home/administrator/Romaco/langchain/lessons/1. RAG/examples/pixegami/PDF_files_langchain/rag-tutorial-v2-main/retrieved.txt"
#     docs = "\n\n---\n\n".join([clean_context(doc.page_content) for doc, _ in results]) if results else "No information found."
#     # Always write to the file, even if nothing is found
#     if not os.path.exists(os.path.dirname(file_path)):
#         os.makedirs(os.path.dirname(file_path))
#     with open(file_path, "w") as f:
#         f.write(docs)
...
#     with RedisSaver.from_conn_string("redis://localhost:6379") as redis_saver:
#         redis_saver.setup()  # Runs the setup once
#         agent_app = create_agent_graph(checkpointer=redis_saver)
